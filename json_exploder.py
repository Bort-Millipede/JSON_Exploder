...
def usage():
	...
	sys.stderr.write("\t--pyramid: Generate a \"pyramid\" list payload: max_indices first level, n-1 all subsequent nested levels until top level with single randomized value.\n")
	...
	sys.stderr.write("\t--fully-random: (Default) Use different random values for all indices in payload.\n")
	...
	sys.stderr.write("\t--full-charset-all: Randomly generate strings (with escaping) AND strings within objects (without Unicode character escaping) including escaped low ASCII (0x00-0x1f), high ASCII (0x7F-0xFF), and Unicode (0x0100-0xFFFF) characters.\n")
	sys.stderr.write("\t-f: Force \"unsafe\" operations (ex. \"pyramid list\" payload with max_indices>4)\n\t\tNOTE: setting -f option will automatically set --outfile option, as payload sizes can increase VERY rapidly!\n")
...
if __name__ == '__main__':
	...
	if max_indices>4 and (pyramid or reverse_pyramid):
		if not unsafe:
			sys.stderr.write("pyramid or reverse payloads with max_indices>4 can only be generated by specifying -f!\n")
			usage()
			sys.exit(7)
